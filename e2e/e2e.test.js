...
describe('Credit Card Validator E2E Tests', () => {
    ...
    beforeAll(async () => {
        ...
        // Ожидание элемента с увеличенным таймаутом
        try {
            await page.waitForSelector('.card-input', { timeout: 30000 });
            console.log('Found .card-input element');
        } catch (error) {
            console.error('Could not find .card-input element:', error);
